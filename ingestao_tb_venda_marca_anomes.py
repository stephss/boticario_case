...
with models.DAG(
        'ingestao_tb_venda_marca_anomes',
        schedule_interval=datetime.timedelta(days=1),
        default_args=default_dag_args) as dag:
    def runner():
        table_id = 'trusted.tb_venda_marca_anomes'
        ...
        groupedData = df.groupby(
            ...
            as_index=False
        ).agg({'QTD_VENDA': 'sum', 'MARCA': 'first', 'ANO': 'first', 'MES': 'first'})
        groupedData.columns = ['total_venda', 'marca', 'ano', 'mes']
        groupedData.to_gbq(table_id, if_exists='replace')


    populate_table = python_operator.PythonOperator(
        task_id='ingestao_tb_venda_marca_anomes',
